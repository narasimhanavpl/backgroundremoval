...
% Description: This code Active Contours algorithm
% Inputs: I           2D image
...
for its = 1:max_its   % Note: no automatic convergence test
    ...
    %-- intermediate checks
    if (mod(its,50) == 0)
        %-- make sure contour is evolving or stop
        se = phi<=0; se = sum(se(:));
        tmp = abs(se-se1);
        if tmp == 0 && its > 50 ,break,end
        if(display),showCurveAndPhi(I,phi,its);end
    end
end
